  ...
  showConfirm(
    ...
    function () {
      const observaciones = document
        .getElementById("confirmObsInput")
        .value.trim();
      $.post(
        "../ajax/rechazar_prestamo.php",
        { id_prestamo: id, observaciones: observaciones },
        function (resp) {
          showToast(resp, "success");
          setTimeout(() => location.reload(), 1200);
        },
...
function _mostrarSelectorCondicion(id) {
  ...
  // 💾 Guardar contenido original SOLO la primera vez
  if (!confirmOriginalHTML) {
    confirmOriginalHTML = box.innerHTML;
  }
...
function _enviarDevolucion(id, condicion) {
  $.post(
    "../ajax/autorizar_devolucion.php",
    { id_prestamo: id, condicion: condicion },
    function (resp) {
      showToast(resp, "success");
      setTimeout(() => location.reload(), 1200);
    },
