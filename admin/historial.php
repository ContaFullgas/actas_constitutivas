...
$historial = mysqli_query($conn, "
    SELECT 
        ...
        u.nombre, u.usuario, u.departamento, p.observaciones, p.condicion_devolucion,
                    ...
                    <div class="form-group">
                        ...
                        <select name="empresa" class="form-select">
                            ...
                            <?php while ($emp = mysqli_fetch_assoc($empresas)) { ?>
                                <option value="<?= $emp['id_empresa'] ?>" <?= ($empresa == $emp['id_empresa']) ? 'selected' : '' ?>>
                                    <?= $emp['nombre_empresa'] ?>
                                </option>
                            <?php } ?>
                        </select>
        ...
        <div class="card">
            <div class="card-title">
                ...
                <?php if ($total_filas > 0): ?>
                    <span style="font-size:12px;font-weight:400;color:var(--text-soft);margin-left:8px"><?= $total_filas ?>
                        registro<?= $total_filas != 1 ? 's' : '' ?></span>
                <?php endif; ?>
            </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    if ($h['estado'] == 'devuelto') {
                                        if ($h['condicion_devolucion'] == 'malo') {
                                            echo '<span class="badge bg-danger">Mal estado</span>';
                                        } else {
                                            echo '<span class="badge bg-success">Buen estado</span>';
                                        }
                                    } else {
                                        echo '<span class="date-empty">—</span>';
                                    }
                                    ?>
                                </td>
                                ...
                                <td>
                                    ...
                                    if ($h['estado'] == 'rechazado') {
                                        echo $h['observaciones'] ?: '<span class="date-empty">Sin motivo</span>';
                                    } else {
                                        echo '<span class="date-empty">—</span>';
                                    }
                                    ?>
                    ...
                    <div class="historial-card">
                        ...
                        <div class="historial-card-meta">
                            ...
                            <span>
                                <?php
                                if ($h['estado'] == 'devuelto') {
                                    echo $h['condicion_devolucion'] == 'malo'
                                        ? 'Mal estado'
                                        : 'Buen estado';
                                } else {
                                    echo '—';
            ...
            <?php if ($total_pags > 1): ?>
                <div class="pagination-wrap">
                    ...
                    <div class="pagination">
                        ...
                        if ($ini > 1) {
                            echo "<a class=\"page-btn\" href=\"?{$filter_prefix}p=1\">1</a>";
                            if ($ini > 2)
                                echo '<span class="page-dots">···</span>';
                        }
                        for ($i = $ini; $i <= $fin; $i++) {
                            $act = $i === $pagina ? 'active' : '';
                            echo "<a class=\"page-btn $act\" href=\"?{$filter_prefix}p=$i\">$i</a>";
                        }
                        if ($fin < $total_pags) {
                            if ($fin < $total_pags - 1)
                                echo '<span class="page-dots">···</span>';
                            echo "<a class=\"page-btn\" href=\"?{$filter_prefix}p={$total_pags}\">{$total_pags}</a>";
                        }
                        ?>
