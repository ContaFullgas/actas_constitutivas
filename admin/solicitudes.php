...
<body>
        ...
        <div class="card">
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <tbody>
                        <?php $num = $num_inicio;
                        while ($s = mysqli_fetch_assoc($solicitudes)) { ?>
                            <tr>
                                ...
                                <td class="center">
                                    <?php if ($s['estado'] == 'pendiente') { ?>
                                        <button class="btn-authorize"
                                            onclick="autorizarPrestamo(<?= $s['id_prestamo'] ?>)">Autorizar prestamo</button>
                                        <button class="btn-reject" onclick="rechazarPrestamo(<?= $s['id_prestamo'] ?>)">Rechazar
                                            prestamo</button>
                                    <?php } ?>
                                    <?php if ($s['estado'] == 'devolucion_pendiente') { ?>
                                        <button class="btn-return"
                                            onclick="autorizarDevolucion(<?= $s['id_prestamo'] ?>)">Autorizar
                                            devolución</button>
                                    <?php } ?>
                                </td>
                            </tr>
            ...
            <div class="card-list">
                <?php mysqli_data_seek($solicitudes, 0);
                $num = $num_inicio;
                while ($s = mysqli_fetch_assoc($solicitudes)) { ?>
                    <div class="solicitud-card">
                        ...
                        <div class="solicitud-card-actions">
                            <?php if ($s['estado'] == 'pendiente') { ?>
                                <button class="btn-authorize" onclick="autorizarPrestamo(<?= $s['id_prestamo'] ?>)">Autorizar
                                    prestamo</button>
                                <button class="btn-reject" onclick="rechazarPrestamo(<?= $s['id_prestamo'] ?>)">Rechazar
                                    prestamo</button>
                            <?php } ?>
                            <?php if ($s['estado'] == 'devolucion_pendiente') { ?>
                                <button class="btn-return" onclick="autorizarDevolucion(<?= $s['id_prestamo'] ?>)">Autorizar
                                    devolución</button>
                            <?php } ?>
                        </div>
                    </div>
            ...
            <?php if ($total_pags > 1): ?>
                <div class="pagination-wrap">
                    <div class="pagination-info">
                        Mostrando <?= $num_inicio ?>–<?= min($offset + $por_pagina, $total_filas) ?> de <?= $total_filas ?>
                        solicitudes
                    </div>
                    <div class="pagination">
                        <a class="page-btn <?= $pagina <= 1 ? 'disabled' : '' ?>"
                            href="?<?= $search_qs ?>p=<?= $pagina - 1 ?>">&#8249;</a>
                        <?php
                        $rango = 2;
                        $ini = max(1, $pagina - $rango);
                        $fin = min($total_pags, $pagina + $rango);
                        if ($ini > 1) {
                            echo "<a class=\"page-btn\" href=\"?{$search_qs}p=1\">1</a>";
                            if ($ini > 2)
                                echo '<span class="page-dots">···</span>';
                        }
                        for ($i = $ini; $i <= $fin; $i++) {
                            $act = $i === $pagina ? 'active' : '';
                            echo "<a class=\"page-btn $act\" href=\"?{$search_qs}p=$i\">$i</a>";
                        }
                        if ($fin < $total_pags) {
                            if ($fin < $total_pags - 1)
                                echo '<span class="page-dots">···</span>';
                            echo "<a class=\"page-btn\" href=\"?{$search_qs}p={$total_pags}\">{$total_pags}</a>";
                        }
                        ?>
    ...
    <script>
        ...
        function cerrarConfirm() {
            ...
            if (confirmOriginalHTML) {
                box.innerHTML = confirmOriginalHTML;
            }

            // 🔁 Reasignar comportamiento del botón confirm
            document.getElementById('confirmOkBtn').onclick = function () { };
        }
        ...
        // 👇 eliminar botones de condición
        const cont = document.getElementById('condicionBtns');
        if (cont) cont.remove();
